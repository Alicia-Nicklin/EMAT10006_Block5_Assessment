...
class Network:
    ...
    def get_clustering(self):
        ...
        for node in self.nodes:
            ...
            for n in neighbours_in: # i have tried having two variables here 'for i,j in' which i think is closer but still not quite correct 
                neighbour_n = self.nodes[n]
                if neighbour_n.connections[n] == 1:
                    true_connections += 1
                    visited_neighbours.append(neighbour_n)
    ...
    def get_mean_clustering(self):
        mean_clustering = sum(self.get_clustering()) / (len(self.nodes))
